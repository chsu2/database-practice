...
p "Solution for 6"
db.execute("SELECT * FROM olympicMedals WHERE medal is 'Silver' OR medal is 'gold' LIMIT 5;"
) do |row|
p row
end 
p "###############################################################"
...
p "Solution for 7"
db.execute("SELECT * FROM olympicMedals WHERE medal IS 'Gold' AND NOT country IS 'United States' LIMIT 5;"
) do |row|
p row
end 
p "###############################################################"
...
p "Solution for 8"
db.execute("SELECT country FROM olympicMedals WHERE year >2000 ORDER BY country ASC LIMIT 5;"
) do |row|
p row
end 
p "###############################################################"
...
p "Solution for 10"
db.execute("SELECT MAX (year), MIN (year) FROM olympicMedals;"
) do |row| 
 p row
end
p "###############################################################"
...
p "Solution for 11"
db.execute("SELECT COUNT ( * ) AS medal, country FROM olympicMedals GROUP BY country ORDER BY medal DESC LIMIT 5;"
) do |row|
 p row
end
p "###############################################################"
...
p "Solution for 14"
db.execute("SELECT COUNT ( olympicMedals.medal ) AS medal, countryLookup.country_name 
FROM olympicMedals 
INNER JOIN countryLookup ON olympicMedals.country=countryLookup.code 
WHERE medal is 'Gold' 
GROUP BY countryLookup.country_name 
...
p "Solution for 15"
db.execute("SELECT COUNT ( olympicMedals.medal ) AS medal, countryLookup.country_name, countryLookup.population
FROM olympicMedals  
INNER JOIN countryLookup ON olympicMedals.country=countryLookup.code 
